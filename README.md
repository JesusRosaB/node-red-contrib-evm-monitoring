...
Currently, there are five nodes implemented:
 ...
 - node-block-log-listener: compilation process of the solidity code
 - node-contract-address-subscription: compilation process from bpmn code to solidity code
 - node-fromaddress-subscription: deployment process of the solidity code
 - node-get-block-details: interaction process with a smart contract already deployed in a blockchain network
 - node-get-transaction-details: interaction process with a smart contract already deployed in a blockchain network by specifying the parameters in the node itself
 - node-toaddress-subscription: interaction process with a smart contract already deployed in a blockchain network by specifying the parameters in the node itself
 - node-transaction-hash-listener: interaction process with a smart contract already deployed in a blockchain network by specifying the parameters in the node itself
 - node-transaction-listener: interaction process with a smart contract already deployed in a blockchain network by specifying the parameters in the node itself
